   ...
   public function update(Request $request, $id)
{
    try {
        ...
        if ($request->hasFile('foto')) {
            ...
            if ($residente->foto && \Storage::disk('public')->exists($residente->foto)) {
                \Storage::disk('public')->delete($residente->foto);
            }

            $fotoPath = $request->file('foto')->store('residentes/fotos', 'public');
            $data['foto'] = $fotoPath;
        }

        $residente = $this->residenteService->actualizarResidente($id, $data);
